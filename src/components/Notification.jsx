...
const Notification = ({ type, message, onClose }) => {
  ...
  const { color, bgColor, iconPath } = types[type];

  return (
    <div className={`fixed top-1/4 left-1/2 transform -translate-x-1/2 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${bgColor}`} role="alert">
      <div className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${color} ${bgColor} rounded-lg dark:bg-green-800 dark:text-green-200`}>
